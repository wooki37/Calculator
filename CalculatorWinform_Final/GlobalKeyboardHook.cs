...
namespace CalculatorWinform
{
    class GlobalKeyboardHook
    {
        ...
        [DllImport("user32.dll")] static extern bool UnhookWindowsHookEx(IntPtr hInstance); [DllImport("user32.dll")]
        static extern int CallNextHookEx(IntPtr idHook, int nCode, int wParam, ref keyboardHookStruct IParam);
        [DllImport("user32.dll")] static extern short GetKeyState(int nCode); [DllImport("kernel32.dll")]
        ...
        public void hook()
        {
            IntPtr hInstance = LoadLibrary("User32"); hhook = SetWindowsHookEx(WH_KEYBOARD_LL, khp, hInstance, 0);
        }
        ...
        public int hookproc(int code, int wParam, ref keyboardHookStruct IParam)
        {
            if (code >= 0)
            {
                ...
                else if ((wParam == WM_KEYUP || wParam == WM_SYSKEYUP) && (KeyUp != null)) { KeyUp(this, kea); }
